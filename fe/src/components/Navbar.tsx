    ...
    <nav className="bg-blue-900 text-white p-3 sm:py-2 sm:px-6 flex flex-col gap-3 shadow-sm sticky top-0 z-50">      
      <div className="w-full flex flex-row gap-2 items-center justify-between sm:grid sm:grid-cols-[120px_1fr_120px] sm:gap-6">
        ...
        <div className="shrink-0 sm:min-w-0">
          <Link to="/" className="text-xl font-bold tracking-wide text-white text-decoration-none block truncate">
            Fotobook
          </Link>
        </div>
        ...
        <div className="flex flex-row items-center justify-end sm:justify-between flex-1 sm:min-w-0">
          ...
          {isLoggedIn && user ? (
            <Link to={`/profile/${user.id}`} className="flex items-center space-x-2 text-white text-decoration-none shrink-0 group">
              <Avatar 
        ...
        <div className="shrink-0 text-right sm:text-center sm:min-w-0">
          {isLoggedIn ? (
            ...
            <Link to="/login" className="text-white bg-orange-500 hover:bg-orange-600 px-3 py-1 sm:py-1.5 rounded-xs transition-colors text-decoration-none text-sm font-bold shadow-2xs">
              Login
            </Link>
          )}
        </div>
