...
export default function AlbumProfileGrid({ items, isOwnProfile }: AlbumProfileGridProps) {
  return (
        ...
        <div key={item.id} className="flex flex-col group min-w-0">
          ...
          <div className="w-full aspect-square bg-gray-100 rounded-md relative shadow-md border-2 border-white overflow-hidden shrink-0">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-black/40 text-white rounded-full flex flex-col items-center justify-center text-center">
              <span className="text-xs font-bold leading-none">{item.photos_count}</span>
              <span className="text-[7px] font-bold tracking-tight">PHOTOS</span>
            </div>
            ...
            {isOwnProfile && (
              <button className="absolute bottom-2 right-2 bg-black/50 text-white text-[9px] font-bold px-1.5 py-0.5 rounded hover:bg-black/70 cursor-pointer z-20">
                EDIT
              </button>
            )}
          </div>
